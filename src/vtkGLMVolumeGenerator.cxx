...
void vtkGLMVolumeGenerator::SimpleExecute(vtkImageData *input, vtkImageData* output)
{
	...
	std::cout<<"vtkGLMVolumeGenerator running simplexecute()"<<std::endl;
    ...
    for (int kk = 0; kk < imgDim[2]; kk++)
    {
        for (int jj = 0; jj < imgDim[1]; jj++)
        {
            for (int ii = 0; ii < imgDim[0]; ii++)
            {
                ...
                rss = scalarsInput->GetComponent(indx, yy++);//chisq
                corrCoeff = scalarsInput->GetComponent(indx, yy);//for progress update bar
