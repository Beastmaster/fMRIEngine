...
public:
	...
	vtkGetMacro(FDRThreshold, float);float vtkGetFDRThreshold() {return this->FDRThreshold;}
    // vtkSetMacro(FDRThreshold, float);
    
    // Description:
    // Gets/Sets the dof 
    vtkGetMacro(DOF, int);
    vtkSetMacro(DOF, int);//void vtkSetDOF(int DOF) {this->DOF=DOF;}
    
    // Description:
    // Gets/Sets the option 
    vtkGetMacro(Option, int);
    vtkSetMacro(Option, int);//void vtkSetOption(int option) {this->Option=option;}

    // Description:
    // Gets/Sets the Q 
    vtkGetMacro(Q, float);
    vtkSetMacro(Q, float);//void vtkSetQ(float q) {this->Q=q;}
